...
export default function ChatbotBubble() {
    ...
    const sendMessage = async (text) => {
        ...
        try {
            ...
            setMessages(prev => [...prev, { sender: 'bot', text: 'Terjadi kesalahan saat menghubungi asisten. Silakan coba lagi.' }]);
        } finally {
            setIsTyping(false);
        }
    };
    ...
    return (
        <>
            ...
            <button
                onClick={toggle}
                aria-label="Buka chatbot GITA"
                className={`fixed ${isOpen ? 'bottom-[calc(550px+32px)]' : 'bottom-6'} right-6 z-[9999] w-14 h-14 rounded-full bg-gradient-to-br from-[#33C8DA] to-[#2BA9B8] text-white shadow-lg flex items-center justify-center transition-all duration-300 hover:scale-110 hover:shadow-[0_6px_25px_rgba(51,200,218,0.6)] animate-[chatPulse_2s_infinite]`}
                style={{ bottom: isOpen ? 'calc(550px + 32px)' : '24px' }}
            >
            ...
            <style>{`
                @keyframes chatPulse {
                    0%   { box-shadow: 0 0 0 0 rgba(51,200,218,0.4); }
                    70%  { box-shadow: 0 0 0 12px rgba(51,200,218,0); }
                    100% { box-shadow: 0 0 0 0 rgba(51,200,218,0); }
                }
                @keyframes fadeIn {
